...
def train() -> None:
    path = ROOT / "data" / "processed" / "ganak_rent_clean.csv"
    df = pd.read_csv(path)
    if len(df) < 2:
        print("not enough rows for rent model")
        return
    ...
    model.fit(df[features], df["deposit"])
    (ROOT / "models").mkdir(exist_ok=True)
    joblib.dump(model, ROOT / "models" / "rent_model.pkl")
    print("saved models/rent_model.pkl")
